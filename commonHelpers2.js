import{A as _,i as c,a as C}from"./assets/quote-889d4774.js";import{i as I,P as X,g as R,a as z}from"./assets/vendor-5014e6d0.js";const Y=document.querySelector("#upbutton");Y.addEventListener("click",K);function K(){(document.body.scrollTop>0||document.documentElement.scrollTop>0)&&window.scrollTo({top:0,behavior:"smooth"})}window.onscroll=()=>{(window.pageYOffset||document.documentElement.scrollTop)>700?document.getElementById("upbutton").style.display="block":document.getElementById("upbutton").style.display="none"};const V=new _,j=document.querySelector(".js-footer-form"),d=document.querySelector(".footer-form-btn"),L=document.querySelector('input[type="email"]');function k(e){return/^[^\s@]+(?:\.[^\s@]+)*@[^\s@]+\.[^\s@]+$/.test(e)}d&&(d.disabled=!0,L.addEventListener("input",()=>{const e=L.value;k(e)?(d.style.backgroundColor="#c6cdd1",d.disabled=!1):(d.style.backgroundColor="#f4f4f4",d.disabled=!0)}),d.addEventListener("mouseover",()=>{const e=L.value;k(e)?d.style.cursor="pointer":d.style.cursor="not-allowed"}),d.addEventListener("mouseout",()=>{d.style.cursor="default"}),j.addEventListener("submit",async e=>{var n,u;e.preventDefault();const t=L.value;if(!k(t)){I.warning({message:"Invalid email address was entered.",position:"topRight"});return}const i={email:t};try{const r=await V.subscription(i);if(console.log("resp",r),((n=r==null?void 0:r.response)==null?void 0:n.status)===409){I.warning({message:"Subscription already exists",position:"topRight"});return}if(((u=r==null?void 0:r.response)==null?void 0:u.status)===400){I.warning({message:"Bad request (invalid request body)",position:"topRight"});return}r!=null&&r.message&&I.success({message:r.message,position:"topRight"})}catch(r){console.log("error",r)}finally{j.reset()}}));const s={paginationBox:document.getElementById("tui-pagination"),categoriesContainer:document.querySelector(".js-categories-container"),categoriesList:document.querySelector(".js-categories-list"),exercise:document.querySelector(".js-exercise"),exerciseDecor:document.querySelector(".js-exercise-decor"),categoryError:document.querySelector(".js-categories-error"),categorySceletonLoader:document.querySelector(".js-sceleton-loader"),search:document.querySelector(".js-search"),exerciseRefresh:document.querySelector(".js-refresh"),searchIcon:document.querySelector(".menu-search-icon"),closeIcon:document.querySelector(".menu-close-icon"),searchBtn:document.querySelector(".menu-search-btn"),searchInput:document.querySelector(".menu-search-input")};s.exerciseRefresh.addEventListener("click",Z);function Z(e){location.reload()}const H=(e,t)=>{const i=e.reduce((n,{filter:u,imgURL:r,name:l})=>(n+=`<li class="categories__card-item js-exercise"
      ...
      </li>`,n),"");t.innerHTML=i},U=(e,t)=>{const i=e.reduce((n,{rating:u,target:r,bodyPart:l,burnedCalories:g,name:m,_id:B,time:P})=>(n+=`<li class="card-item" id=${B}>
    <div class="card-menu">
    ...
    <div class="card-info">
      <div class="card-info-about">
        ...
        <div class="card-info-about-value card-time">${g} / ${P} min</div>
      </div>
      <div class="card-info-about">
  ...
  </li>`,n),"");t.innerHTML=i};class ee{constructor(){this.pagination}init(t,i,n){let u=3;const r={totalItems:i*n,itemsPerPage:n,visiblePages:u,template:{page:'<a href="#" class="tui-page-btn">{{page}}</a>',currentPage:'<span class="tui-page-btn tui-is-selected">{{page}}</span>',moveButton:'<a href="#" class="tui-page-btn tui-{{type}}"><span class="tui-ico-{{type}}"></span></a>',disabledMoveButton:'<span class="tui-page-btn tui-is-disabled tui-{{type}}"><span class="tui-ico-{{type}}"></span></span>',moreButton:'<a href="#" class="tui-page-btn tui-{{type}}-is-ellip"><span class="tui-ico-ellip">...</span></a>'}};this.pagination=new X(s.paginationBox,r),this.pagination.on("afterMove",async({page:l})=>{if(console.log("service",t),t.type===C.FILTERS){t.setPage(l);const g=await t.getCategories();H(g.results,s.categoriesContainer)}if(t.type===C.EXEECISES){t.setPage(l);const g=await t.getExercises();U(g.results,s.categoriesContainer)}})}}const F=new ee,T=new _(C.FILTERS),y=new _(C.EXEECISES);s.categoriesList.addEventListener("click",ie);s.search.addEventListener("submit",e=>ae(e,w));s.searchInput.addEventListener("input",e=>te(e,w));s.searchInput.addEventListener("focus",se);s.searchInput.addEventListener("blur",N);const w={searchQuery:""};s.closeIcon&&s.closeIcon.addEventListener("click",J);document.addEventListener("click",function(e){e.target.closest("#menu-close-icon")&&J(e)});function te(e,t){const i=e.currentTarget.value.trim();t.searchQuery!==i&&(s.searchBtn.innerHTML=`
      <svg id="menu-search-icon" class="menu-search-icon" width="18" height="18">
        <use href="${c}#icon-search"></use>
      </svg>
    `)}function se(e){e.target.placeholder=""}function N(e){e.target.value.trim()||(e.target.placeholder="Search")}async function J(e){s.searchBtn.innerHTML=`
      <svg id="menu-search-icon" class="menu-search-icon" width="18" height="18">
        <use href="${c}#icon-search"></use>
      </svg>
    `,s.search.reset();const t=new FormData(s.search);w.searchQuery=t.get("search").trim();const i=y.getExercise();y.setSearch(w.searchQuery),w.searchQuery===void 0&&N(e),await A(i),s.categoryError.classList.add("visually-hidden")}async function ae(e,t){e.preventDefault();const i=new FormData(e.currentTarget);t.searchQuery=i.get("search").trim(),t.searchQuery!==""&&(s.searchBtn.innerHTML=`
      <svg id="menu-close-icon" class="menu-close-icon" width="18" height="18">
        <use href="${c}#icon-close"></use>
      </svg>
    `),s.categoryError.classList.add("visually-hidden");const n=y.getExercise();return console.log(n),y.setSearch(t.searchQuery),await A(n),t.searchQuery}async function ie(e){if(e.target.nodeName!=="LI")return;s.exercise.textContent="",s.exerciseDecor.classList.add("visually-hidden"),s.categoryError.classList.add("visually-hidden"),s.search.classList.add("visually-hidden");const t=e.currentTarget.querySelector(".current");t&&t.classList.remove("current"),e.target.classList.add("current");const i=e.target.textContent;T.setCategory(i),y.setCategory(i),await G()}async function G(){const e=await T.getCategories();if(s.categorySceletonLoader.classList.add("visually-hidden"),H(e.results,s.categoriesContainer),e!=null&&e.totalPages||s.categoryError.classList.remove("visually-hidden"),e.totalPages<=1){s.paginationBox.classList.add("visually-hidden");return}s.paginationBox.classList.remove("visually-hidden"),F.init(T,e.totalPages,e.perPage)}G();s.categoriesContainer.addEventListener("click",re);async function re(e){if(e.target.closest(".js-exercise")){const t=e.target.closest(".js-exercise").dataset.exercise;s.exercise.textContent=t,s.exerciseDecor.classList.remove("visually-hidden"),s.search.classList.remove("visually-hidden"),await A(t);return}}async function A(e){y.setExercise(e);const t=await y.getExercises();if(U(t.results,s.categoriesContainer),t!=null&&t.totalPages||s.categoryError.classList.remove("visually-hidden"),t.totalPages<=1){s.paginationBox.classList.add("visually-hidden");return}s.paginationBox.classList.remove("visually-hidden"),F.init(y,t.totalPages,t.perPage)}const O=["#Sport","#Healthy","#Workout","#Diet"],Q=100,oe=document.querySelectorAll(".hero-item-text");let $=0;function W(){if($<O.length){const e=O[$],t=oe[$];let i=0;const n=setInterval(function(){i<e.length?(t.textContent+=e[i],i++):(clearInterval(n),$++,setTimeout(W,Q))},Q)}}W();const ne={root:null,rootMargin:"0px",threshold:0},M=document.querySelector(".footer-title-span"),ce=new IntersectionObserver(le,ne);ce.observe(M);function le(e,t){e.forEach(i=>{i.isIntersecting&&(R.set(M,{clearProps:"all"}),R.to(M,{duration:2,opacity:1,x:0,rotationX:360,delay:0}))})}function de(){const e=document.getElementsByClassName("categories__cards-list"),t=document.getElementById("modal");let i=!1;function n(){document.body.style.overflow="hidden"}function u(){document.body.style.overflow=""}function r(){t.style.display="flex",n(),i=!0}function l(){t.style.display="none",u(),i=!1}document.querySelector(".modal-main");const g=document.getElementsByClassName("modal-close-btn")[0];let m=localStorage.getItem("savedExercises")?JSON.parse(localStorage.getItem("savedExercises")):[];g&&(g.onclick=function(){l(),localStorage.removeItem("currentExercise")}),window.onclick=function(a){a.target==t&&(l(),localStorage.removeItem("currentExercise"))},document.addEventListener("keydown",function(a){a.key==="Escape"&&i&&(l(),localStorage.removeItem("currentExercise"))});for(let a=0;a<e.length;a+=1)e[a].addEventListener("click",function(p){const v=p.target.closest(".card-menu-start"),b=p.target.closest(".card-item");if(v){const x=b.id;r();const f=`https://your-energy.b.goit.study/api/exercises/${x}`;z.get(f).then(o=>{if(o)return localStorage.setItem("currentExercise",JSON.stringify(o.data)),o.data}).then(o=>{console.log(o),B(o);const h=document.getElementById("fav-btn");if(h){const D=m.some(S=>S._id===o._id);h.querySelector(".modal-heart-icon use").setAttribute("href",D?`${c}#icon-trash`:`${c}#icon-heart`),h.querySelector(".modal-btn-text").textContent=D?"Remove from favorites":"Add to favorites",h.addEventListener("click",function(){const S=m.some(q=>q._id===o._id);S?m=m.filter(q=>q._id!==o._id):m.push(o),localStorage.setItem("savedExercises",JSON.stringify(m)),h.querySelector(".modal-btn-text").textContent=S?"Add to favorites":"Remove from favorites",h.querySelector(".modal-heart-icon use").setAttribute("href",S?`${c}#icon-heart`:`${c}#icon-trash`)})}}).catch(o=>{console.error("There was a problem with the Axios request:",o)})}});function B(a){const E=document.getElementById("cardDetails");console.log("exerciseDetailsContainer",E);let p=null;a.gifUrl?p=`<div class="modal-gif-wrapper">
        ...
        </div>`;const v=`<p class='modal-card-title'>${a.name.charAt(0).toUpperCase()+a.name.slice(1)}</p>`,b=P(Math.round(a.rating*10)/10),x=`<div class='modal-rating-container'>
      ...
      </div>`;E.innerHTML=`${p+`<div class="modal-content-wrapper">${v+x+f+o+h}</div>`}`}function P(a){const E=`<div class="modal-star-wrapper"><svg class="modal-star-icon modal-colored-star">
      <use href="${c}#icon-star"></use>
