import{P as M,a as b}from"./vendor-5c458d1f.js";(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))c(t);new MutationObserver(t=>{for(const s of t)if(s.type==="childList")for(const r of s.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&c(r)}).observe(document,{childList:!0,subtree:!0});function o(t){const s={};return t.integrity&&(s.integrity=t.integrity),t.referrerpolicy&&(s.referrerPolicy=t.referrerpolicy),t.crossorigin==="use-credentials"?s.credentials="include":t.crossorigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function c(t){if(t.ep)return;t.ep=!0;const s=o(t);fetch(t.href,s)}})();const d="/mvp-your-energy/assets/sprite-c1fb1bf2.svg",S={paginationBox:document.getElementById("tui-pagination"),categoriesContainer:document.querySelector(".js-categories-container"),categoriesList:document.querySelector(".js-categories-list"),exercise:document.querySelector(".js-exercise"),exerciseDecor:document.querySelector(".js-exercise-decor"),categoryError:document.querySelector(".js-categories-error"),categorySceletonLoader:document.querySelector(".js-sceleton-loader"),search:document.querySelector(".js-search"),exerciseRefresh:document.querySelector(".js-refresh"),searchIcon:document.querySelector(".menu-search-icon"),closeIcon:document.querySelector(".menu-close-icon"),searchBtn:document.querySelector(".menu-search-btn"),searchInput:document.querySelector(".menu-search-input"),exerciseFavorites:document.querySelector(".js-exercise-favorites")},k=(i,e)=>{const o=i.reduce((c,{filter:t,imgURL:s,name:r})=>(c+=`<li class="categories__card-item js-exercise"
      ...
      </li>`,c),"");e.innerHTML=o},T=(i,e)=>{const o=i.reduce((c,{rating:t,target:s,bodyPart:r,burnedCalories:l,name:u,_id:$,time:w})=>(c+=`<li class="card-item" id=${$}>
    <div class="card-menu">
    ...
    <div class="card-info">
      ...
      <div class="card-info-about">
  ...
  </li>`,c),"");e.innerHTML=o};function P(i){return i.map(({_id:e,target:o,bodyPart:c,burnedCalories:t,time:s,name:r})=>`
        ...
        `).join("")}const E="https://your-energy.b.goit.study/api",L={FILTERS:"filters",EXEECISES:"exercises"},O={Muscles:"muscles","Body parts":"bodypart",Equipment:"equipment"},A="savedExercises";class K{constructor(){this.pagination}init(e,o,c){let t=3;const s={totalItems:o*c,itemsPerPage:c,visiblePages:t,template:{page:'<a href="#" class="tui-page-btn">{{page}}</a>',currentPage:'<span class="tui-page-btn tui-is-selected">{{page}}</span>',moveButton:'<a href="#" class="tui-page-btn tui-{{type}}"><span class="tui-ico-{{type}}"></span></a>',disabledMoveButton:'<span class="tui-page-btn tui-is-disabled tui-{{type}}"><span class="tui-ico-{{type}}"></span></span>',moreButton:'<a href="#" class="tui-page-btn tui-{{type}}-is-ellip"><span class="tui-ico-ellip">...</span></a>'}};this.pagination=new M(S.paginationBox,s),this.pagination.on("afterMove",async({page:r})=>{if(!e){const l=JSON.parse(localStorage.getItem(A))??[];S.exerciseFavorites.innerHTML=P(l.slice((r-1)*c,r*c));return}if(e.type===L.FILTERS){e.setPage(r);const l=await e.getCategories();k(l.results,S.categoriesContainer)}if(e.type===L.EXEECISES){e.setPage(r);const l=await e.getExercises();T(l.results,S.categoriesContainer)}})}}(()=>{const i=document.querySelector(".js-mobile-menu"),e=document.querySelector(".js-open-menu"),o=document.querySelector(".js-close-menu"),c=document.querySelectorAll(".mobile-social-box"),t=document.querySelector("body"),s=()=>{const l=e.getAttribute("aria-expanded")==="true"||!1;e.setAttribute("aria-expanded",!l),i.classList.toggle("is-open"),t.classList.toggle("disable-scroll")},r=l=>l.forEach(u=>{u.addEventListener("click",s)});e.addEventListener("click",s),o.addEventListener("click",s),r(c)})();const D=document.querySelector("#upbutton");D.addEventListener("click",_);function _(){(document.body.scrollTop>0||document.documentElement.scrollTop>0)&&window.scrollTo({top:0,behavior:"smooth"})}window.onscroll=()=>{(window.pageYOffset||document.documentElement.scrollTop)>700?document.getElementById("upbutton").style.display="block":document.getElementById("upbutton").style.display="none"};class j{constructor(e){this.type=e,this.category="Muscles",this.exercise="",this.page=1,this.search=""}setCategory(e){this.category=e}setPage(e){this.page=e}setExercise(e){this.exercise=e}getExercise(){return this.exercise}setSearch(e){this.search=e}async getCategories(){try{return(await b.get(`${E}/filters?filter=${this.category}&page=${this.page}&perPage=12`)).data}catch(e){return console.log(e),e}}async getExercises(){try{const e=await b.get(`${E}/exercises?${O[this.category]}=${this.exercise}&keyword=${this.search}&page=${this.page}&perPage=12`);return console.log(e.data),e.data}catch(e){return console.log(e),e}}async getQuoteOfTheDay(){try{return(await b.get(`${E}/quote`)).data}catch(e){return console.log(e),e}}async subscription(e){try{return(await b.post(`${E}/subscription`,e)).data}catch(o){return o}}}class R{constructor(){}getData(e){try{const o=localStorage.getItem(e);return JSON.parse(o)}catch{console.error("Get error: ",err)}}setData(e,o){try{const c=JSON.stringify(o);localStorage.setItem(e,c)}catch{console.error("Set error: ",err)}}removeData(e){try{localStorage.removeItem(e)}catch{console.error("Remove error: ",err)}}}const N=new j,B=new R,U=document.querySelector(".quote-text-js"),F=document.querySelector(".quote-author-js");async function J(){const i=new Date,e=i.getDate(),o=i.getMonth()+1,c=i.getFullYear(),t=`${e}.${o}.${c}`,s=B.getData("quote");if(s&&s.date===t)C(s.data);else try{const r=await N.getQuoteOfTheDay(),l={date:t,data:r};B.setData("quote",l),C(r)}catch(r){console.error("Error fetching the quote:",r)}}function C(i){U.textContent=i.quote,F.textContent=i.author}J();function H(){const i=document.getElementsByClassName("categories_favorites_cards"),e=document.getElementById("modal");let o=!1;function c(){document.body.style.overflow="hidden"}function t(){document.body.style.overflow=""}function s(){e.style.display="flex",c(),o=!0}function r(){e.style.display="none",t(),o=!1}document.querySelector(".modal-main");const l=document.getElementsByClassName("modal-close-btn")[0];let u=localStorage.getItem("savedExercises")?JSON.parse(localStorage.getItem("savedExercises")):[];l&&(l.onclick=function(){r(),localStorage.removeItem("currentExercise")}),window.onclick=function(a){a.target==e&&(r(),localStorage.removeItem("currentExercise"))},document.addEventListener("keydown",function(a){a.key==="Escape"&&o&&(r(),localStorage.removeItem("currentExercise"))});for(let a=0;a<i.length;a+=1)i[a].addEventListener("click",function(g){const m=g.target.closest(".card-menu-start"),f=g.target.closest(".card-item");if(m){const h=f.id;s();const p=`https://your-energy.b.goit.study/api/exercises/${h}`;b.get(p).then(n=>{if(n)return localStorage.setItem("currentExercise",JSON.stringify(n.data)),n.data}).then(n=>{$(n);const v=document.getElementById("fav-btn");if(v){const I=u.some(y=>y._id===n._id);v.querySelector(".modal-heart-icon use").setAttribute("href",I?`${d}#icon-trash`:`${d}#icon-heart`),v.querySelector(".modal-btn-text").textContent=I?"Remove from favorites":"Add to favorites",v.addEventListener("click",function(){const y=u.some(q=>q._id===n._id);y?u=u.filter(q=>q._id!==n._id):u.push(n),localStorage.setItem("savedExercises",JSON.stringify(u)),v.querySelector(".modal-btn-text").textContent=y?"Add to favorites":"Remove from favorites",v.querySelector(".modal-heart-icon use").setAttribute("href",y?`${d}#icon-heart`:`${d}#icon-trash`)})}}).catch(n=>{console.error("There was a problem with the Axios request:",n)})}});function $(a){const x=document.getElementById("cardDetails");let g=null;a.gifUrl?g=`<div class="modal-gif-wrapper">
      ...
      <use href="${d}#icon-star"></use>
          ...
          <use href="${d}#icon-star"></use>
          </svg></div>`}else m+=g;return`${m}`}}document.addEventListener("DOMContentLoaded",function(){H()});export{j as A,K as C,A as L,T as a,L as b,P as c,k as g,d as i,S as r};
//# sourceMappingURL=modal-7e5e0330.js.map
